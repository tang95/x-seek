...
func init() {
	serverCmd.PersistentFlags().StringVarP(&cfgFile, "config", "c", "", "config file (default is $HOME/.x-seek/server.yaml)")
}
...
func readConfig() (*config.Server, error) {
	if cfgFile != "" {
		...
		viper.SetConfigFile(filepath.Join(home, ".x-seek", "server.yaml"))
	}
	cfg := &config.Server{}
...
func newServer() ([]Server, error) {
	...
	svc, err := service.NewService(cfg, logger, d.IncidentRepo, d.Transaction, d.UserRepo, d.TeamRepo)
	if err != nil {
		return nil, err
	}
